...
export default function FundamentalsQuizRunner() {
	...
	useEffect(() => {
		(async () => {
			...
			if (!me) {
				router.push(`/login?next=/ai-fundamentals/quiz/${quizId}`);
				return;
			}
			try {
				const res = await apiFetch(`/fundamentals/quiz/${quizId}`);
	...
	async function submitAnswer() {
		...
		try {
			...
			const res = await apiFetch(`/fundamentals/quiz/${quizId}/answer`, {
				method: "POST",
	...
	async function finish() {
		await apiFetch(`/fundamentals/quiz/${quizId}/complete`, { method: "POST" });
		router.push(`/ai-fundamentals/quiz/${quizId}/results`);
	}
		...
		return (
			<main className="min-h-dvh bg-[#0a0a0a] flex items-center justify-center px-4">
				<div className="max-w-md text-center">
					...
					<Link href="/ai-fundamentals/quiz" className="font-sans text-[13px] text-[#ec4899] hover:underline">Back to launcher →</Link>
				</div>
			</main>
		);
	}
		...
		<main className="min-h-dvh bg-[#0a0a0a] text-[#e8e8e8]">
			...
			<div className="max-w-[760px] mx-auto px-6 py-10">
				<div className="flex flex-wrap gap-2 mb-5">
					<span className="px-2 py-0.5 rounded font-sans text-[11px] font-medium border" style={{ color: "#ec4899", borderColor: "#ec489940", backgroundColor: "#ec48990D" }}>
						{question.topicLabel}
					</span>
				...
				{!reveal && (
					<>
						<button
							...
							className="w-full py-3.5 rounded-xl bg-[#ec4899] text-white font-sans text-[15px] font-bold transition-colors hover:bg-[#db2777] disabled:opacity-40 disabled:cursor-not-allowed"
						>
				...
				{reveal && (
					<div className="space-y-4">
						...
						<button
							...
							className="w-full py-3.5 rounded-xl bg-[#ec4899] text-white font-sans text-[15px] font-bold hover:bg-[#db2777]"
						>
